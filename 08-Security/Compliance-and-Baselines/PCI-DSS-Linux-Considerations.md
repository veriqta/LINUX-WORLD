# PCI DSS Linux Considerations

This guide explains pci dss linux considerations as part of Linux defensive security. It connects the security objective to safe administration, verification, monitoring and recovery.
...
- Explain how pci dss linux considerations supports risk-based control baselines and defensible evidence.
...
In an isolated VM, document the current pci dss linux considerations state, identify one defensible improvement, predict its operational effect, implement it through a safe change, verify positive and negative cases, then restore the original state. Submit the threat model, commands, evidence, rollback result and remaining risk.

## Review questions

1. Which threat does pci dss linux considerations reduce, and which threats remain?
