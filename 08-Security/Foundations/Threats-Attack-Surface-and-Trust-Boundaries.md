# Threats Attack Surface and Trust Boundaries

This guide explains threats attack surface and trust boundaries as part of Linux defensive security. It connects the security objective to safe administration, verification, monitoring and recovery.
...
- Explain how threats attack surface and trust boundaries supports security architecture, assets, identities and trust boundaries.
- Identify threats to system state, business data and administrative control.
...
The protected scope includes system state, business data and administrative control. Important failure modes include implicit trust, excessive privilege and missing ownership. Consider external attackers, compromised user accounts, malicious or mistaken insiders, vulnerable services, failed automation and unavailable dependencies.
...
- Define the exact asset, trust boundary and failure consequence for threats attack surface and trust boundaries.
- Record the current state before changing system state, business data and administrative control.
...
Common evidence sources include /etc, /proc, /sys and service configuration. Exact paths and service names differ by distribution and installed software. Record package versions and inspect local manual pages before relying on an example.
...
In an isolated VM, document the current threats attack surface and trust boundaries state, identify one defensible improvement, predict its operational effect, implement it through a safe change, verify positive and negative cases, then restore the original state. Submit the threat model, commands, evidence, rollback result and remaining risk.

## Review questions

1. Which threat does threats attack surface and trust boundaries reduce, and which threats remain?
