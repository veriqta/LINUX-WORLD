...
This guide explains file and configuration change detection as part of Linux defensive security. It connects the security objective to safe administration, verification, monitoring and recovery.
...
- Explain how file and configuration change detection supports observable evidence and actionable security detections.
- Identify threats to logs, audit events, processes, persistence and network activity.
...
The protected scope includes logs, audit events, processes, persistence and network activity. Important failure modes include blind spots, unactionable alerts, tampered logs and missed context. Consider external attackers, compromised user accounts, malicious or mistaken insiders, vulnerable services, failed automation and unavailable dependencies.
...
- Define the exact asset, trust boundary and failure consequence for file and configuration change detection.
- Record the current state before changing logs, audit events, processes, persistence and network activity.
...
Common evidence sources include journal, audit logs, authentication logs and monitoring rules. Exact paths and service names differ by distribution and installed software. Record package versions and inspect local manual pages before relying on an example.
...
In an isolated VM, document the current file and configuration change detection state, identify one defensible improvement, predict its operational effect, implement it through a safe change, verify positive and negative cases, then restore the original state. Submit the threat model, commands, evidence, rollback result and remaining risk.

## Review questions

1. Which threat does file and configuration change detection reduce, and which threats remain?
