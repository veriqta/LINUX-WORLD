...
This guide explains audit rule verification as part of Linux defensive security. It connects the security objective to safe administration, verification, monitoring and recovery.
...
- Explain how audit rule verification supports security-relevant event capture and review.
...
- Define the exact asset, trust boundary and failure consequence for audit rule verification.
...
In an isolated VM, document the current audit rule verification state, identify one defensible improvement, predict its operational effect, implement it through a safe change, verify positive and negative cases, then restore the original state. Submit the threat model, commands, evidence, rollback result and remaining risk.

## Review questions

1. Which threat does audit rule verification reduce, and which threats remain?
