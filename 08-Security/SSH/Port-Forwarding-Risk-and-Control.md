# Port Forwarding Risk and Control

This guide explains port forwarding risk and control as part of Linux defensive security. It connects the security objective to safe administration, verification, monitoring and recovery.
...
- Explain how port forwarding risk and control supports remote administration trust, authentication and channel control.
...
- Preserve console access and a second privileged session.
- Validate candidate configuration with sshd -t and inspect effective values with sshd -T.
- Open a new connection and test both allowed and denied cases before closing the old session.
...
In an isolated VM, document the current port forwarding risk and control state, identify one defensible improvement, predict its operational effect, implement it through a safe change, verify positive and negative cases, then restore the original state. Submit the threat model, commands, evidence, rollback result and remaining risk.

## Review questions

1. Which threat does port forwarding risk and control reduce, and which threats remain?
