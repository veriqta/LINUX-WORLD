# AppArmor for Services and Containers

This guide explains apparmor for services and containers as part of Linux defensive security. It connects the security objective to safe administration, verification, monitoring and recovery.
...
- Explain how apparmor for services and containers supports path-based mandatory access control.
- Identify threats to application profiles, abstractions and mediated resources.
...
The protected scope includes application profiles, abstractions and mediated resources. Important failure modes include unconfined applications, broad rules and ignored denials. Consider external attackers, compromised user accounts, malicious or mistaken insiders, vulnerable services, failed automation and unavailable dependencies.
...
- Do not leave a required profile unconfined as a permanent fix.
- Confirm the correct profile attachment, path resolution and abstraction rules.
- Use complain mode only as a controlled diagnostic step, then restore enforcement.
...
Common evidence sources include /etc/apparmor.d and kernel or journal messages. Exact paths and service names differ by distribution and installed software. Record package versions and inspect local manual pages before relying on an example.
...
In an isolated VM, document the current apparmor for services and containers state, identify one defensible improvement, predict its operational effect, implement it through a safe change, verify positive and negative cases, then restore the original state. Submit the threat model, commands, evidence, rollback result and remaining risk.

## Review questions

1. Which threat does apparmor for services and containers reduce, and which threats remain?
