# Minimal Installation and Attack Surface

This guide explains minimal installation and attack surface as part of Linux defensive security. It connects the security objective to safe administration, verification, monitoring and recovery.
...
- Explain how minimal installation and attack surface supports attack-surface reduction with tested operational controls.
- Identify threats to boot integrity, services, packages, filesystems and logs.
...
The protected scope includes boot integrity, services, packages, filesystems and logs. Important failure modes include unsafe defaults, unused components and configuration drift. Consider external attackers, compromised user accounts, malicious or mistaken insiders, vulnerable services, failed automation and unavailable dependencies.
...
- Define the exact asset, trust boundary and failure consequence for minimal installation and attack surface.
- Record the current state before changing boot integrity, services, packages, filesystems and logs.
- Apply the smallest control that satisfies the requirement, then test authorized and unauthorized behaviour.
...
Common evidence sources include /etc/sysctl.d, /etc/systemd, /etc/fstab and package sources. Exact paths and service names differ by distribution and installed software. Record package versions and inspect local manual pages before relying on an example.
...
In an isolated VM, document the current minimal installation and attack surface state, identify one defensible improvement, predict its operational effect, implement it through a safe change, verify positive and negative cases, then restore the original state. Submit the threat model, commands, evidence, rollback result and remaining risk.

## Review questions

1. Which threat does minimal installation and attack surface reduce, and which threats remain?
...
- https://docs.redhat.com/en/documentation/red_hat_enterprise_linux/9/html/security_hardening/
