# SELinux for Services and Containers

This guide explains selinux for services and containers as part of Linux defensive security. It connects the security objective to safe administration, verification, monitoring and recovery.
...
- Explain how selinux for services and containers supports label-based mandatory access control.
...
- Do not permanently disable SELinux to solve a denial.
- Confirm the application action is legitimate, then inspect domain, type, label and policy.
- Prefer restorecon, semanage and approved booleans before generating local policy.
...
In an isolated VM, document the current selinux for services and containers state, identify one defensible improvement, predict its operational effect, implement it through a safe change, verify positive and negative cases, then restore the original state. Submit the threat model, commands, evidence, rollback result and remaining risk.

## Review questions

1. Which threat does selinux for services and containers reduce, and which threats remain?
