# Dependency and Artifact Risk

This guide explains dependency and artifact risk as part of Linux defensive security. It connects the security objective to safe administration, verification, monitoring and recovery.
...
- Explain how dependency and artifact risk supports software provenance and unauthorized-change detection.
- Identify threats to packages, repositories, artifacts, builds and configuration.
...
The protected scope includes packages, repositories, artifacts, builds and configuration. Important failure modes include tampered artifacts, untrusted repositories and hidden drift. Consider external attackers, compromised user accounts, malicious or mistaken insiders, vulnerable services, failed automation and unavailable dependencies.
...
- Define the exact asset, trust boundary and failure consequence for dependency and artifact risk.
- Record the current state before changing packages, repositories, artifacts, builds and configuration.
...
Common evidence sources include package keys, repository definitions and integrity baselines. Exact paths and service names differ by distribution and installed software. Record package versions and inspect local manual pages before relying on an example.
...
In an isolated VM, document the current dependency and artifact risk state, identify one defensible improvement, predict its operational effect, implement it through a safe change, verify positive and negative cases, then restore the original state. Submit the threat model, commands, evidence, rollback result and remaining risk.

## Review questions

1. Which threat does dependency and artifact risk reduce, and which threats remain?
