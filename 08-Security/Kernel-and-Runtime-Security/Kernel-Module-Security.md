...
This guide explains kernel module security as part of Linux defensive security. It connects the security objective to safe administration, verification, monitoring and recovery.
...
- Explain how kernel module security supports kernel interfaces and runtime isolation controls.
...
- Define the exact asset, trust boundary and failure consequence for kernel module security.
...
In an isolated VM, document the current kernel module security state, identify one defensible improvement, predict its operational effect, implement it through a safe change, verify positive and negative cases, then restore the original state. Submit the threat model, commands, evidence, rollback result and remaining risk.

## Review questions

1. Which threat does kernel module security reduce, and which threats remain?
