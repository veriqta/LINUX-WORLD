# Core Dumps and Debugging Risk

This guide explains core dumps and debugging risk as part of Linux defensive security. It connects the security objective to safe administration, verification, monitoring and recovery.
...
- Explain how core dumps and debugging risk supports kernel interfaces and runtime isolation controls.
- Identify threats to kernel modules, processes, memory, capabilities and system calls.
...
The protected scope includes kernel modules, processes, memory, capabilities and system calls. Important failure modes include dangerous capabilities, broad syscall access and untrusted modules. Consider external attackers, compromised user accounts, malicious or mistaken insiders, vulnerable services, failed automation and unavailable dependencies.
...
- Define the exact asset, trust boundary and failure consequence for core dumps and debugging risk.
- Record the current state before changing kernel modules, processes, memory, capabilities and system calls.
...
Common evidence sources include /proc, /sys, /etc/modprobe.d and systemd unit restrictions. Exact paths and service names differ by distribution and installed software. Record package versions and inspect local manual pages before relying on an example.
...
In an isolated VM, document the current core dumps and debugging risk state, identify one defensible improvement, predict its operational effect, implement it through a safe change, verify positive and negative cases, then restore the original state. Submit the threat model, commands, evidence, rollback result and remaining risk.

## Review questions

1. Which threat does core dumps and debugging risk reduce, and which threats remain?
