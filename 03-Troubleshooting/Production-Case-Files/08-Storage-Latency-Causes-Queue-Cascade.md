# Storage Latency Causes a Queue Cascade

> Educational production case. Names, times, and identifiers are intentionally generic.

## Incident summary

Request latency, worker backlog, and retries increased across several services.
...
A storage volume reached its performance limit. Synchronous writes slowed, workers blocked, queues grew, and retries multiplied load.
...
Shed noncritical work, limited retries, redirected writes to healthy capacity, and drained queues gradually.
...
Alert on latency and queue depth, model storage limits, cap retries, isolate critical I/O, and rehearse failover.
