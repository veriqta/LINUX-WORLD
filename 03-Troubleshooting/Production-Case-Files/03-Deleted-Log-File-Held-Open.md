# Deleted Log File Held Open

> Educational production case. Names, times, and identifiers are intentionally generic.

## Incident summary

The root filesystem stayed full after a large log file was deleted. New writes and package operations failed.
...
The application retained an open file descriptor to the deleted inode, so the blocks were not released.
...
Reduced traffic, confirmed the descriptor with `lsof +L1`, used the application's supported log reopen action, and verified capacity recovery.
...
Use correct log rotation, test reopen behavior, alert on deleted-open files, and set retention and size limits.
