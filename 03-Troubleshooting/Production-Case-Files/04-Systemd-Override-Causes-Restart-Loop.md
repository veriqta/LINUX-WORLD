# systemd Override Causes a Restart Loop

> Educational production case. Names, times, and identifiers are intentionally generic.

## Incident summary

A service restarted continuously after a configuration rollout, consuming CPU and flooding logs.
...
A drop-in override replaced the expected environment and caused immediate exit. Aggressive restart policy amplified the failure.
...
Stopped the restart loop, captured the effective unit, reverted the drop-in, reloaded systemd, and restored service with a controlled start.
...
Validate effective unit configuration, canary drop-ins, rate-limit restarts, and test required environment explicitly.
