# Inode Exhaustion on an Application Host

> Educational production case. Names, times, and identifiers are intentionally generic.

## Incident summary

New sessions and deployments failed on one application host. Existing processes continued serving cached requests.
...
A temporary-file cleanup regression created millions of small files. Byte capacity remained available, but the filesystem exhausted inodes.
...
Removed the host from traffic, stopped the producer, archived a small diagnostic sample, deleted verified disposable files in controlled batches, and returned the host after validation.
...
Add inode alerts, bound temporary-file creation, test cleanup jobs, monitor file counts, and use a dedicated filesystem for high-churn temporary data.
