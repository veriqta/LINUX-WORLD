...
Only some clients, record types, nameservers, or time windows experience lookup failures.
...
```bash
targeted `dig` against each server with TCP and trace
resolver logs
packet capture
...
Route around the bad resolver or correct delegation, transport, or record inconsistency.
...
All intended resolver paths return consistent authoritative answers within latency objectives.
