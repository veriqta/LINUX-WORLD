# Memory Pressure and OOM Selection

## Scenario

The kernel or cgroup kills workloads during sustained or burst memory pressure.
...
cgroup `memory.current`, `memory.events`, `memory.stat`
kernel logs
process RSS and maps
```
...
Protect critical workloads, reduce allocation or concurrency, and correct limits with capacity evidence.
...
No new OOM events occur under representative load and reclaim or swap does not create unacceptable latency.
