...
A systemd unit enters failed state or exits during startup.
...
Correct the validated cause, run `systemctl daemon-reload` when unit files changed, then start once.
...
The unit is active, health checks pass, logs remain clean, and restart behavior is correct.
