...
The shell cannot resolve a command that should be available.
...
Use the distribution package manager or correct the controlled PATH. Never download an unknown binary as a shortcut.
...
A new login resolves the expected executable and `COMMAND --version` reports the intended version.
