...
A package manager cannot refresh, resolve, install, or configure a package.
...
```bash
`cat /etc/os-release`
package-manager diagnostic output
`df -hT`
`ps -ef | grep -E 'apt|dpkg|dnf|rpm'`
DNS and HTTPS test
```
...
Fix the repository, lock owner, dependency, or capacity issue. Do not delete lock files while a valid transaction is active.
...
Package database is consistent, the intended version is installed, and dependent services pass checks.
