...
A command reports `No such file or directory`, or an expected path appears missing.
...
Correct the path or restore the expected mount or link. Do not create a replacement until the intended source is confirmed.
...
The correct path resolves, permissions are unchanged, and the application reads the intended object.
