...
A process remains after a stop request or returns immediately.
...
```bash
`pgrep -af NAME`
`ps -o pid,ppid,user,stat,wchan,etime,cmd -p PID`
`pstree -ap PID`
...
Send SIGTERM first. Fix the supervisor or blocked dependency. Use SIGKILL only as a last resort.
...
The process exits, its supervisor remains in the intended state, and required cleanup completes.
