...
A connection is refused, times out, or has no route.
...
Correct the failed layer. Keep remote access available when changing routes or firewall policy.
...
The intended client completes the application request, not only a ping or port test.
