...
A logical volume requires safe expansion without confusing device, LV, and filesystem layers.
...
Extend the correct LV and filesystem using supported tools. Never shrink as part of an emergency unless a tested procedure exists.
...
`lvs`, `findmnt`, and `df` agree on the new capacity and the application writes successfully.
