# Runner Users Groups and Permissions

This article explains runner users groups and permissions from the Linux operations perspective. It focuses on the host, identities, processes, files, network, storage, automation, telemetry and recovery responsibilities that make the capability safe to operate.
...
- Use short-lived workload identity where the platform supports it.
- Do not place secrets in source, command lines, logs, images, plans or public examples.
- Test both authorized access and explicit denial.
...
1. What Linux responsibility remains when using runner users groups and permissions?
