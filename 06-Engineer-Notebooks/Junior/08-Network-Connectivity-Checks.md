...
Document a connection from name resolution through routing, socket and application response.
...
### 2. Address, prefix and interface state
...
### 4. DNS result and resolver path
...
### 5. Listening socket and firewall state
...
### 6. Application protocol and return-path result
...
### Connectivity path worksheet

| Source | Destination | DNS result | Route/interface | Port | Firewall decision | TLS/application result |
| --- | --- | --- | --- | --- | --- | --- |
|  |  |  |  |  |  |  |

**Coverage requirement:** Test the path in layers: local link, address, route, name resolution, transport, TLS and application.
...
- [ ] Risk reviewed: Relying only on ping.
- [ ] Risk reviewed: Testing wrong source.
- [ ] Risk reviewed: Publishing public addresses.
- [ ] Risk reviewed: Changing firewall without console.
