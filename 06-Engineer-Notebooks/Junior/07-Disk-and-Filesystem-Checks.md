...
Record block-device, filesystem, capacity and persistence checks without risking data.
...
### 2. Filesystem type, UUID and mount point
...
### 3. Block use, inode use and growth
...
### 4. Mount options and fstab entry
...
### 5. Open deleted files and hidden mount data
...
### 6. Read-only state, errors and backup status
...
### Storage health worksheet

| Device/LV | Filesystem | UUID | Mount point | Blocks used | Inodes used | Errors |
| --- | --- | --- | --- | --- | --- | --- |
|  |  |  |  |  |  |  |

| Persistent definition | Expected options | Observed options | Snapshot/backup | Recovery note |
| --- | --- | --- | --- | --- |
|  |  |  |  |  |

**Coverage requirement:** Trace the stack from device to filesystem and mount; inspect capacity, inodes, read-only state, kernel errors and persistence.
...
- [ ] Risk reviewed: Wrong device target.
- [ ] Risk reviewed: Formatting during diagnosis.
- [ ] Risk reviewed: Ignoring inodes.
- [ ] Risk reviewed: Rebooting with untested fstab.
