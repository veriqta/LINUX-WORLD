# Secure Hosting Platform

## Capstone challenge

Create a multi-tenant Linux hosting environment with hardened access, service isolation, network policy, resource limits, secret and certificate handling, audit evidence, backup, and incident response.

## Scope

This capstone combines architecture, Linux administration, automation, security, reliability, observability, troubleshooting, recovery, governance, and technical communication.

## Core domains

- SSH certificates or controlled keys
- Systemd isolation
- Cgroups
- Firewall
- Filesystem permissions
- Auditing
- Vulnerability management
- And forensics readiness.

## Required deliverables

- Threat model
- Tenant model
- Hardened baseline
- Access platform
- Isolation controls
- Resource governance
- Audit
- Recovery
- And security exercise.

## Documents

- [Project Brief](PROJECT-BRIEF.md)
- [Implementation Guide](IMPLEMENTATION-GUIDE.md)
- [Acceptance Tests](ACCEPTANCE-TESTS.md)
- [Operations Runbook](OPERATIONS-RUNBOOK.md)
- [Portfolio Notes](PORTFOLIO-NOTES.md)

## Capstone rule

+## Executable capstone files

- [Architecture](ARCHITECTURE.md)
- [Prerequisites](PREREQUISITES.md)
- [Milestone Guide](MILESTONE-GUIDE.md)
- [Integration Map](INTEGRATION-MAP.md)
- [Security Review](SECURITY-REVIEW.md)
- [SLO and Capacity Plan](SLO-CAPACITY-PLAN.md)
- [Game-Day Guide](GAME-DAY-GUIDE.md)
- [Demonstration Guide](DEMONSTRATION-GUIDE.md)
- [Final Assessment](FINAL-ASSESSMENT.md)
- [Cleanup and Decommissioning](CLEANUP.md)
- [Portfolio Presentation](PRESENTATION.md)
- [Automation](scripts/)
- [Configuration](config/)
- [Tests](tests/)
- [Reference Framework](solutions/)
- [Decision Records](adrs/)
- [Game-Day Records](game-days/)
- [Evidence Guidance](evidence/)



No architecture claim is accepted without evidence. Define the environment, scale, objective, measurement, tested failure, and limitation.
