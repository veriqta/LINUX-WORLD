# Linux Health Check

## Project summary

Build a portable health-check tool that evaluates essential host signals and returns clear status, evidence, and exit codes.

## Level

Junior. This project emphasizes correct single-host administration, safe execution, verification, and clear documentation.

## Skills developed

- Bash functions
- Exit status
- Thresholds
- Process and service checks
- Formatted output

## Core deliverables

- Health-check script
- Configuration
- Sample output
- Tests
- Installation guide
- And monitoring integration notes

## Project documents

- [Project Brief](PROJECT-BRIEF.md)
- [Implementation Guide](IMPLEMENTATION-GUIDE.md)
- [Acceptance Tests](ACCEPTANCE-TESTS.md)
- [Operations Runbook](OPERATIONS-RUNBOOK.md)
- [Portfolio Notes](PORTFOLIO-NOTES.md)

## Completion standard

+
## Hands-on learning files

- [Architecture](ARCHITECTURE.md)
- [Prerequisites](PREREQUISITES.md)
- [Environment Setup](ENVIRONMENT-SETUP.md)
- [Step-by-Step Guide](STEP-BY-STEP-GUIDE.md)
- [Commands Explained](COMMANDS-EXPLAINED.md)
- [Troubleshooting](TROUBLESHOOTING.md)
- [Cleanup](CLEANUP.md)
- [Interview Questions](INTERVIEW-QUESTIONS.md)
- [Working Scripts](scripts/)
- [Configuration Examples](config/)
- [Expected Output](examples/)
- [Executable Tests](tests/)
- [Reference Solution](solutions/)
- [Architecture Diagram](assets/architecture-diagram.md)



The build must be reproducible on a clean disposable system. Functional, negative, security, failure, recovery, persistence, and cleanup tests must pass. Public evidence must be sanitized.
