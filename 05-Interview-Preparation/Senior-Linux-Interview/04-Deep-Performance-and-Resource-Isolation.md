# Deep Performance and Resource Isolation

> Level: Senior System Administrator

## Scope

scheduler classes, CPU affinity, NUMA locality, huge pages, memory reclaim, IO schedulers, cgroup v2 and PSI.

## Interview questions and model answers

### 1. What should a strong Senior candidate understand about Deep Performance and Resource Isolation?

A strong answer connects scheduler classes, CPU affinity, NUMA locality, huge pages, memory reclaim, IO schedulers, cgroup v2 and PSI. It defines each idea, explains how the pieces interact, and finishes with a way to observe the real system rather than relying on memory.

### 2. Which commands would you use first?

Begin with read-only evidence: `ps -eLo pid,tid,psr,cls,rtprio,ni,stat,comm; taskset -cp PID; numastat -p PID; memory.stat; io.stat; perf record`. Select only the commands relevant to the symptom, narrow their scope, record time and host context, and explain what result would support or reject the current hypothesis.
...
Warning signs include Benchmarking unlike workloads, pinning without topology awareness, hiding contention with larger limits. A strong candidate recognizes uncertainty, refuses unsafe shortcuts, and proposes a bounded test.
...
Prove the limiting resource, isolate a noisy neighbor, validate tail latency and check for shifted bottlenecks. Require the candidate to narrate risk, select evidence, perform the work in an isolated lab, verify the intended condition, test one failure and restore the original state.
...
**Task:** Prove the limiting resource, isolate a noisy neighbor, validate tail latency and check for shifted bottlenecks.
