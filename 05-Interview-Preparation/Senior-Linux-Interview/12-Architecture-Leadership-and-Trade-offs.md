# Architecture Leadership and Trade-offs

> Level: Senior System Administrator

## Scope

requirements, constraints, failure domains, operability, security, cost, migration, ownership, governance and review.

## Interview questions and model answers

### 1. What should a strong Senior candidate understand about Architecture Leadership and Trade-offs?

A strong answer connects requirements, constraints, failure domains, operability, security, cost, migration, ownership, governance and review. It defines each idea, explains how the pieces interact, and finishes with a way to observe the real system rather than relying on memory.

### 2. Which commands would you use first?

Begin with read-only evidence: `architecture decision records, threat models, SLO documents, capacity models, game-day reports and runbooks`. Select only the commands relevant to the symptom, narrow their scope, record time and host context, and explain what result would support or reject the current hypothesis.
...
Warning signs include Tool-first design, hidden operational work, centralized single points, migrations without exit criteria. A strong candidate recognizes uncertainty, refuses unsafe shortcuts, and proposes a bounded test.
...
Present a Linux platform design, defend trade-offs, identify unknowns and define how decisions will be validated. Require the candidate to narrate risk, select evidence, perform the work in an isolated lab, verify the intended condition, test one failure and restore the original state.
...
**Task:** Present a Linux platform design, defend trade-offs, identify unknowns and define how decisions will be validated.
