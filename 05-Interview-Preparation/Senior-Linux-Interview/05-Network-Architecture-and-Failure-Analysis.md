# Network Architecture and Failure Analysis

> Level: Senior System Administrator

## Scope

routing domains, ECMP, conntrack, NAT, MTU, offload, queues, DNS, TLS, load balancing and asymmetric paths.

## Interview questions and model answers

### 1. What should a strong Senior candidate understand about Network Architecture and Failure Analysis?

A strong answer connects routing domains, ECMP, conntrack, NAT, MTU, offload, queues, DNS, TLS, load balancing and asymmetric paths. It defines each idea, explains how the pieces interact, and finishes with a way to observe the real system rather than relying on memory.

### 2. Which commands would you use first?

Begin with read-only evidence: `ip route get; ss -ti; nstat; ethtool -S IFACE; conntrack -S; tcpdump; tracepath; dig +trace; openssl s_client`. Select only the commands relevant to the symptom, narrow their scope, record time and host context, and explain what result would support or reject the current hypothesis.
...
Warning signs include One-sided captures, disabling offload blindly, exhausting conntrack, retry amplification, DNS cache confusion. A strong candidate recognizes uncertainty, refuses unsafe shortcuts, and proposes a bounded test.
...
Diagnose regional connection resets using client, host, network and load-balancer evidence. Require the candidate to narrate risk, select evidence, perform the work in an isolated lab, verify the intended condition, test one failure and restore the original state.
...
**Task:** Diagnose regional connection resets using client, host, network and load-balancer evidence.
