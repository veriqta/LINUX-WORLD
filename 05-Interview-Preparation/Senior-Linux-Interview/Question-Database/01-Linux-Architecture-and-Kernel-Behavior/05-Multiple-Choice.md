# Linux Architecture and Kernel Behavior: Multiple-Choice Bank

Choose the best answer, then explain why every other option is weaker.

## 1. A candidate needs to investigate system calls safely. What is the best first approach?

A. Restart the service immediately

B. Run `strace -f -ttT -p PID`, define the expected evidence and preserve the before state
...
## 2. A scheduler change works now but must survive reboot. Which evidence is strongest?
...
## 3. Users are denied while an administrator succeeds in virtual memory. What should be tested next?
...
## 4. Which result best proves recovery of page cache?
...
## 5. A measurement related to interrupts is high. What is the best interpretation?
...
## 6. Which change practice is safest for NUMA?
...
## 7. A candidate needs to investigate cgroups safely. What is the best first approach?

A. Restart the service immediately

B. Run `sysctl -a`, define the expected evidence and preserve the before state
...
## 8. A namespaces and kernel interfaces change works now but must survive reboot. Which evidence is strongest?
