# Observability and Evidence Strategy: True or False

## 1. If a command related to telemetry purpose exits successfully, the complete requirement is proven.
...
## 2. A configuration file for RED and USE methods can be correct while runtime behavior remains wrong.

**Answer:** True. The process may use another source, cached or generated state, different precedence or a failed reload.

**Correction exercise:** State the evidence required to demonstrate the corrected claim.

## 3. Testing high cardinality only as root is sufficient.

**Answer:** False. Root can bypass controls and hide identity-specific failures. Test with the intended account or client.

**Correction exercise:** State the evidence required to demonstrate the corrected claim.

## 4. A safe sampling change needs a defined rollback trigger before implementation.
...
## 6. A negative test is useful when validating time.

**Answer:** True. It proves prohibited behavior remains denied and can expose overly broad access.

**Correction exercise:** State the evidence required to demonstrate the corrected claim.

## 7. If a command related to provenance and evidence integrity exits successfully, the complete requirement is proven.
