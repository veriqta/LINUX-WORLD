...
severity, roles, timeline, evidence, containment, mitigation, recovery, communication, decision log and postmortem.

## Interview questions and model answers

### 1. What should a strong Senior candidate understand about Incident Command and Recovery?

A strong answer connects severity, roles, timeline, evidence, containment, mitigation, recovery, communication, decision log and postmortem. It defines each idea, explains how the pieces interact, and finishes with a way to observe the real system rather than relying on memory.

### 2. Which commands would you use first?

Begin with read-only evidence: `date -u +%FT%TZ; last -x; journalctl --since; dmesg -T; ss; ip; lsof; package history; configuration diff`. Select only the commands relevant to the symptom, narrow their scope, record time and host context, and explain what result would support or reject the current hypothesis.
...
Warning signs include Hero debugging, silent changes, no user validation, premature root cause, action items without owners. A strong candidate recognizes uncertainty, refuses unsafe shortcuts, and proposes a bounded test.
...
Command a major Linux incident, balance restoration with evidence, and manage risky decisions explicitly. Require the candidate to narrate risk, select evidence, perform the work in an isolated lab, verify the intended condition, test one failure and restore the original state.
...
**Task:** Command a major Linux incident, balance restoration with evidence, and manage risky decisions explicitly.
