# Linux Architecture and Kernel Behavior

> Level: Senior System Administrator

## Scope

system calls, scheduler, virtual memory, page cache, interrupts, NUMA, cgroups, namespaces and kernel interfaces.

## Interview questions and model answers

### 1. What should a strong Senior candidate understand about Linux Architecture and Kernel Behavior?

A strong answer connects system calls, scheduler, virtual memory, page cache, interrupts, NUMA, cgroups, namespaces and kernel interfaces. It defines each idea, explains how the pieces interact, and finishes with a way to observe the real system rather than relying on memory.

### 2. Which commands would you use first?

Begin with read-only evidence: `strace -f -ttT -p PID; perf top; slabtop; numastat; cat /proc/PID/smaps_rollup; bpftool prog list; sysctl -a`. Select only the commands relevant to the symptom, narrow their scope, record time and host context, and explain what result would support or reject the current hypothesis.
...
Warning signs include Changing global kernel tunables without workload evidence, treating one snapshot as a trend. A strong candidate recognizes uncertainty, refuses unsafe shortcuts, and proposes a bounded test.
...
Explain a latency regression across user space, system calls, scheduling, memory, IO and dependencies. Require the candidate to narrate risk, select evidence, perform the work in an isolated lab, verify the intended condition, test one failure and restore the original state.
...
**Task:** Explain a latency regression across user space, system calls, scheduling, memory, IO and dependencies.
