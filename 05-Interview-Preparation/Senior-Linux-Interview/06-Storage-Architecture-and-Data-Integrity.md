...
block stack, multipath, RAID, LVM, filesystem journal, write cache, barriers, latency, corruption and durability.

## Interview questions and model answers

### 1. What should a strong Senior candidate understand about Storage Architecture and Data Integrity?

A strong answer connects block stack, multipath, RAID, LVM, filesystem journal, write cache, barriers, latency, corruption and durability. It defines each idea, explains how the pieces interact, and finishes with a way to observe the real system rather than relying on memory.

### 2. Which commands would you use first?

Begin with read-only evidence: `lsblk -o NAME,KNAME,TYPE,FSTYPE,MOUNTPOINTS; multipath -ll; mdadm --detail; dmsetup ls --tree; iostat -xz; smartctl -a`. Select only the commands relevant to the symptom, narrow their scope, record time and host context, and explain what result would support or reject the current hypothesis.
...
Warning signs include Assuming fsync means durable through every layer, unsafe cache policy, repair before imaging evidence. A strong candidate recognizes uncertainty, refuses unsafe shortcuts, and proposes a bounded test.
...
Respond to rising IO latency and integrity errors while protecting evidence and minimizing data loss. Require the candidate to narrate risk, select evidence, perform the work in an isolated lab, verify the intended condition, test one failure and restore the original state.
...
**Task:** Respond to rising IO latency and integrity errors while protecting evidence and minimizing data loss.
