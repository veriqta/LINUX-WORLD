# Security Architecture and Host Defense

> Level: Senior System Administrator

## Scope

identity boundary, privilege path, secure boot, kernel hardening, MAC, secrets, audit, integrity, supply chain and response.

## Interview questions and model answers

### 1. What should a strong Senior candidate understand about Security Architecture and Host Defense?

A strong answer connects identity boundary, privilege path, secure boot, kernel hardening, MAC, secrets, audit, integrity, supply chain and response. It defines each idea, explains how the pieces interact, and finishes with a way to observe the real system rather than relying on memory.

### 2. Which commands would you use first?

Begin with read-only evidence: `mokutil --sb-state; sysctl; getcap -r PATH; systemd-analyze security; ausearch; auditctl -l; keyctl show; package verification`. Select only the commands relevant to the symptom, narrow their scope, record time and host context, and explain what result would support or reject the current hypothesis.
...
Warning signs include Compliance-only controls, uncontrolled break-glass access, alerting without response, secrets in process arguments. A strong candidate recognizes uncertainty, refuses unsafe shortcuts, and proposes a bounded test.
...
Design a hardened administration path with just-in-time access, audit, recovery and emergency controls. Require the candidate to narrate risk, select evidence, perform the work in an isolated lab, verify the intended condition, test one failure and restore the original state.
...
**Task:** Design a hardened administration path with just-in-time access, audit, recovery and emergency controls.
