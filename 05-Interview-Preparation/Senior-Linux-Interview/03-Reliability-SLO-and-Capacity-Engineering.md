# Reliability SLO and Capacity Engineering

> Level: Senior System Administrator

## Scope

SLI, SLO, error budget, burn rate, queueing, utilization, saturation, headroom, failure domains and load shedding.

## Interview questions and model answers

### 1. What should a strong Senior candidate understand about Reliability SLO and Capacity Engineering?

A strong answer connects SLI, SLO, error budget, burn rate, queueing, utilization, saturation, headroom, failure domains and load shedding. It defines each idea, explains how the pieces interact, and finishes with a way to observe the real system rather than relying on memory.

### 2. Which commands would you use first?

Begin with read-only evidence: `sar; pidstat; iostat -xz; ss -s; tc -s qdisc; systemd-cgtop; stress-ng; fio with an authorized test file`. Select only the commands relevant to the symptom, narrow their scope, record time and host context, and explain what result would support or reject the current hypothesis.
...
Warning signs include Averages hiding tails, synthetic load harming production, alerts without action, capacity based on CPU alone. A strong candidate recognizes uncertainty, refuses unsafe shortcuts, and proposes a bounded test.
...
Create a capacity model and multi-window alert policy tied to a user journey and safe scaling action. Require the candidate to narrate risk, select evidence, perform the work in an isolated lab, verify the intended condition, test one failure and restore the original state.
...
**Task:** Create a capacity model and multi-window alert policy tied to a user journey and safe scaling action.
