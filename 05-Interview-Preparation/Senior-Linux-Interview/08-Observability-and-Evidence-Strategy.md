# Observability and Evidence Strategy

> Level: Senior System Administrator

## Scope

telemetry purpose, RED and USE methods, high cardinality, sampling, retention, time, provenance and evidence integrity.

## Interview questions and model answers

### 1. What should a strong Senior candidate understand about Observability and Evidence Strategy?

A strong answer connects telemetry purpose, RED and USE methods, high cardinality, sampling, retention, time, provenance and evidence integrity. It defines each idea, explains how the pieces interact, and finishes with a way to observe the real system rather than relying on memory.

### 2. Which commands would you use first?

Begin with read-only evidence: `journalctl -o json; coredumpctl info; prometheus query examples; bpftrace; sar; chronyc sources; sha256sum EVIDENCE`. Select only the commands relevant to the symptom, narrow their scope, record time and host context, and explain what result would support or reject the current hypothesis.
...
Warning signs include Collecting everything, dashboards without hypotheses, telemetry gaps during overload, altering evidence. A strong candidate recognizes uncertainty, refuses unsafe shortcuts, and proposes a bounded test.
...
Define minimum viable telemetry for a critical Linux service and its dependency failure modes. Require the candidate to narrate risk, select evidence, perform the work in an isolated lab, verify the intended condition, test one failure and restore the original state.
...
**Task:** Define minimum viable telemetry for a critical Linux service and its dependency failure modes.
