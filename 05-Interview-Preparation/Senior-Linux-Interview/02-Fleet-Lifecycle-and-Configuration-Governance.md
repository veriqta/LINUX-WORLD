# Fleet Lifecycle and Configuration Governance

> Level: Senior System Administrator

## Scope

golden image, bootstrap, desired state, drift, rollout ring, immutable versus mutable hosts, inventory and decommissioning.

## Interview questions and model answers

### 1. What should a strong Senior candidate understand about Fleet Lifecycle and Configuration Governance?

A strong answer connects golden image, bootstrap, desired state, drift, rollout ring, immutable versus mutable hosts, inventory and decommissioning. It defines each idea, explains how the pieces interact, and finishes with a way to observe the real system rather than relying on memory.

### 2. Which commands would you use first?

Begin with read-only evidence: `systemd-delta; rpm -Va; debsums -s; osqueryi; ansible-inventory --graph; git diff; journalctl --verify`. Select only the commands relevant to the symptom, narrow their scope, record time and host context, and explain what result would support or reject the current hypothesis.
...
Warning signs include Snowflake hosts, global rollout, weak identity, unmanaged exceptions, incomplete decommission. A strong candidate recognizes uncertainty, refuses unsafe shortcuts, and proposes a bounded test.
...
Design a 10,000-host change with canaries, health gates, rollback, exception handling and auditability. Require the candidate to narrate risk, select evidence, perform the work in an isolated lab, verify the intended condition, test one failure and restore the original state.
...
**Task:** Design a 10,000-host change with canaries, health gates, rollback, exception handling and auditability.
