# Distributed Systems on Linux

> Level: Senior System Administrator

## Scope

timeouts, retries, backoff, idempotency, quorum, leader election, split brain, consistency, clock and backpressure.

## Interview questions and model answers

### 1. What should a strong Senior candidate understand about Distributed Systems on Linux?

A strong answer connects timeouts, retries, backoff, idempotency, quorum, leader election, split brain, consistency, clock and backpressure. It defines each idea, explains how the pieces interact, and finishes with a way to observe the real system rather than relying on memory.

### 2. Which commands would you use first?

Begin with read-only evidence: `ss -ti; tc netem in a lab; chronyc tracking; dig; curl --trace-time; systemd-run; coredumpctl; tcpdump`. Select only the commands relevant to the symptom, narrow their scope, record time and host context, and explain what result would support or reject the current hypothesis.
...
Warning signs include Cascading retries, coordinated omission, clock assumptions, queues without limits, health checks causing load. A strong candidate recognizes uncertainty, refuses unsafe shortcuts, and proposes a bounded test.
...
Explain how a local host symptom can originate in a remote dependency and how to prove causality. Require the candidate to narrate risk, select evidence, perform the work in an isolated lab, verify the intended condition, test one failure and restore the original state.
...
**Task:** Explain how a local host symptom can originate in a remote dependency and how to prove causality.
