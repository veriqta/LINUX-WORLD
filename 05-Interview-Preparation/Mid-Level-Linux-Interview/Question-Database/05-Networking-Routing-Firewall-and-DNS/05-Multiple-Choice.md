# Networking Routing Firewall and DNS: Multiple-Choice Bank

Choose the best answer, then explain why every other option is weaker.

## 1. A candidate needs to investigate CIDR safely. What is the best first approach?

A. Restart the service immediately

B. Run `ip route get ADDRESS`, define the expected evidence and preserve the before state
...
## 2. A policy routing change works now but must survive reboot. Which evidence is strongest?
...
## 3. Users are denied while an administrator succeeds in neighbor discovery. What should be tested next?
...
## 4. Which result best proves recovery of TCP state?
...
## 6. Which change practice is safest for bonding?
...
## 7. A candidate needs to investigate VLAN safely. What is the best first approach?

A. Restart the service immediately

B. Run `nft list ruleset`, define the expected evidence and preserve the before state
...
## 8. A nftables change works now but must survive reboot. Which evidence is strongest?
...
## 9. Users are denied while an administrator succeeds in DNS delegation and caching. What should be tested next?
