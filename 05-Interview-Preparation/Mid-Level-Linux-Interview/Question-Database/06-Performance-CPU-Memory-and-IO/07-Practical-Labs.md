# Performance CPU Memory and IO: Practical Labs

Run only in an authorized disposable environment. Each lab requires a before state, implementation, independent verification, controlled failure, recovery and cleanup.

## Lab 1: load

1. Record the current load state and relevant versions.
2. Define a measurable target related to Performance CPU Memory and IO.
3. Implement the smallest reversible change.
4. Validate syntax or configuration before activation.
5. Prove intended behavior using an identity or client different from the administrator.
6. Prove prohibited behavior remains denied.
7. Introduce one bounded failure and predict its evidence.
8. Detect and recover without using an undocumented shortcut.
9. Reboot or recreate where persistence matters.
10. Clean up and prove unrelated state remains intact.

**Suggested evidence:** `mpstat -P ALL 1`, `pidstat 1`, `vmstat 1`, `iostat -xz 1`, `cat /proc/pressure/{cpu,memory,io}`, `sar`, `perf stat COMMAND`, configuration diff, timestamped logs, exit status and end-to-end result.

**Scoring:** accuracy 3, safety 2, verification 2, recovery 1, explanation 1, cleanup 1.

## Lab 2: run queue

1. Record the current run queue state and relevant versions.
2. Define a measurable target related to Performance CPU Memory and IO.
3. Implement the smallest reversible change.
4. Validate syntax or configuration before activation.
5. Prove intended behavior using an identity or client different from the administrator.
6. Prove prohibited behavior remains denied.
7. Introduce one bounded failure and predict its evidence.
8. Detect and recover without using an undocumented shortcut.
9. Reboot or recreate where persistence matters.
10. Clean up and prove unrelated state remains intact.

**Suggested evidence:** `mpstat -P ALL 1`, `pidstat 1`, `vmstat 1`, `iostat -xz 1`, `cat /proc/pressure/{cpu,memory,io}`, `sar`, `perf stat COMMAND`, configuration diff, timestamped logs, exit status and end-to-end result.

**Scoring:** accuracy 3, safety 2, verification 2, recovery 1, explanation 1, cleanup 1.

## Lab 3: context switch

1. Record the current context switch state and relevant versions.
2. Define a measurable target related to Performance CPU Memory and IO.
3. Implement the smallest reversible change.
4. Validate syntax or configuration before activation.
5. Prove intended behavior using an identity or client different from the administrator.
6. Prove prohibited behavior remains denied.
7. Introduce one bounded failure and predict its evidence.
8. Detect and recover without using an undocumented shortcut.
9. Reboot or recreate where persistence matters.
10. Clean up and prove unrelated state remains intact.

**Suggested evidence:** `mpstat -P ALL 1`, `pidstat 1`, `vmstat 1`, `iostat -xz 1`, `cat /proc/pressure/{cpu,memory,io}`, `sar`, `perf stat COMMAND`, configuration diff, timestamped logs, exit status and end-to-end result.

**Scoring:** accuracy 3, safety 2, verification 2, recovery 1, explanation 1, cleanup 1.

## Lab 4: CPU steal

1. Record the current CPU steal state and relevant versions.
2. Define a measurable target related to Performance CPU Memory and IO.
3. Implement the smallest reversible change.
4. Validate syntax or configuration before activation.
5. Prove intended behavior using an identity or client different from the administrator.
6. Prove prohibited behavior remains denied.
7. Introduce one bounded failure and predict its evidence.
8. Detect and recover without using an undocumented shortcut.
9. Reboot or recreate where persistence matters.
10. Clean up and prove unrelated state remains intact.

**Suggested evidence:** `mpstat -P ALL 1`, `pidstat 1`, `vmstat 1`, `iostat -xz 1`, `cat /proc/pressure/{cpu,memory,io}`, `sar`, `perf stat COMMAND`, configuration diff, timestamped logs, exit status and end-to-end result.

**Scoring:** accuracy 3, safety 2, verification 2, recovery 1, explanation 1, cleanup 1.

## Lab 5: PSI

1. Record the current PSI state and relevant versions.
2. Define a measurable target related to Performance CPU Memory and IO.
3. Implement the smallest reversible change.
4. Validate syntax or configuration before activation.
5. Prove intended behavior using an identity or client different from the administrator.
6. Prove prohibited behavior remains denied.
7. Introduce one bounded failure and predict its evidence.
8. Detect and recover without using an undocumented shortcut.
9. Reboot or recreate where persistence matters.
10. Clean up and prove unrelated state remains intact.

**Suggested evidence:** `mpstat -P ALL 1`, `pidstat 1`, `vmstat 1`, `iostat -xz 1`, `cat /proc/pressure/{cpu,memory,io}`, `sar`, `perf stat COMMAND`, configuration diff, timestamped logs, exit status and end-to-end result.

**Scoring:** accuracy 3, safety 2, verification 2, recovery 1, explanation 1, cleanup 1.

## Lab 6: page cache

1. Record the current page cache state and relevant versions.
2. Define a measurable target related to Performance CPU Memory and IO.
3. Implement the smallest reversible change.
4. Validate syntax or configuration before activation.
5. Prove intended behavior using an identity or client different from the administrator.
6. Prove prohibited behavior remains denied.
7. Introduce one bounded failure and predict its evidence.
8. Detect and recover without using an undocumented shortcut.
9. Reboot or recreate where persistence matters.
10. Clean up and prove unrelated state remains intact.

**Suggested evidence:** `mpstat -P ALL 1`, `pidstat 1`, `vmstat 1`, `iostat -xz 1`, `cat /proc/pressure/{cpu,memory,io}`, `sar`, `perf stat COMMAND`, configuration diff, timestamped logs, exit status and end-to-end result.

**Scoring:** accuracy 3, safety 2, verification 2, recovery 1, explanation 1, cleanup 1.

## Lab 7: swap

1. Record the current swap state and relevant versions.
2. Define a measurable target related to Performance CPU Memory and IO.
3. Implement the smallest reversible change.
4. Validate syntax or configuration before activation.
5. Prove intended behavior using an identity or client different from the administrator.
6. Prove prohibited behavior remains denied.
7. Introduce one bounded failure and predict its evidence.
8. Detect and recover without using an undocumented shortcut.
9. Reboot or recreate where persistence matters.
10. Clean up and prove unrelated state remains intact.

**Suggested evidence:** `mpstat -P ALL 1`, `pidstat 1`, `vmstat 1`, `iostat -xz 1`, `cat /proc/pressure/{cpu,memory,io}`, `sar`, `perf stat COMMAND`, configuration diff, timestamped logs, exit status and end-to-end result.

**Scoring:** accuracy 3, safety 2, verification 2, recovery 1, explanation 1, cleanup 1.

## Lab 8: OOM

1. Record the current OOM state and relevant versions.
2. Define a measurable target related to Performance CPU Memory and IO.
3. Implement the smallest reversible change.
4. Validate syntax or configuration before activation.
5. Prove intended behavior using an identity or client different from the administrator.
6. Prove prohibited behavior remains denied.
7. Introduce one bounded failure and predict its evidence.
8. Detect and recover without using an undocumented shortcut.
9. Reboot or recreate where persistence matters.
10. Clean up and prove unrelated state remains intact.

**Suggested evidence:** `mpstat -P ALL 1`, `pidstat 1`, `vmstat 1`, `iostat -xz 1`, `cat /proc/pressure/{cpu,memory,io}`, `sar`, `perf stat COMMAND`, configuration diff, timestamped logs, exit status and end-to-end result.

**Scoring:** accuracy 3, safety 2, verification 2, recovery 1, explanation 1, cleanup 1.

## Lab 9: disk latency

1. Record the current disk latency state and relevant versions.
2. Define a measurable target related to Performance CPU Memory and IO.
3. Implement the smallest reversible change.
4. Validate syntax or configuration before activation.
5. Prove intended behavior using an identity or client different from the administrator.
6. Prove prohibited behavior remains denied.
7. Introduce one bounded failure and predict its evidence.
8. Detect and recover without using an undocumented shortcut.
9. Reboot or recreate where persistence matters.
10. Clean up and prove unrelated state remains intact.

**Suggested evidence:** `mpstat -P ALL 1`, `pidstat 1`, `vmstat 1`, `iostat -xz 1`, `cat /proc/pressure/{cpu,memory,io}`, `sar`, `perf stat COMMAND`, configuration diff, timestamped logs, exit status and end-to-end result.

**Scoring:** accuracy 3, safety 2, verification 2, recovery 1, explanation 1, cleanup 1.

## Lab 10: queue depth and saturation

1. Record the current queue depth and saturation state and relevant versions.
2. Define a measurable target related to Performance CPU Memory and IO.
3. Implement the smallest reversible change.
4. Validate syntax or configuration before activation.
5. Prove intended behavior using an identity or client different from the administrator.
6. Prove prohibited behavior remains denied.
7. Introduce one bounded failure and predict its evidence.
8. Detect and recover without using an undocumented shortcut.
9. Reboot or recreate where persistence matters.
10. Clean up and prove unrelated state remains intact.

**Suggested evidence:** `mpstat -P ALL 1`, `pidstat 1`, `vmstat 1`, `iostat -xz 1`, `cat /proc/pressure/{cpu,memory,io}`, `sar`, `perf stat COMMAND`, configuration diff, timestamped logs, exit status and end-to-end result.

**Scoring:** accuracy 3, safety 2, verification 2, recovery 1, explanation 1, cleanup 1.
