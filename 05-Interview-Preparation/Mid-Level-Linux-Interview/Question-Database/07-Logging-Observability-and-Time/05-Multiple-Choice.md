# Logging Observability and Time: Multiple-Choice Bank

Choose the best answer, then explain why every other option is weaker.

## 1. A candidate needs to investigate structured logs safely. What is the best first approach?

A. Restart the service immediately

B. Run `journalctl --list-boots`, define the expected evidence and preserve the before state
...
## 2. A journald persistence change works now but must survive reboot. Which evidence is strongest?
...
## 3. Users are denied while an administrator succeeds in rsyslog routing. What should be tested next?
...
## 4. Which result best proves recovery of rotation?
...
## 5. A measurement related to metrics is high. What is the best interpretation?
...
## 6. Which change practice is safest for cardinality?
...
## 7. A candidate needs to investigate alerting and clock synchronization safely. What is the best first approach?

A. Restart the service immediately

B. Run `ss -ulpn`, define the expected evidence and preserve the before state
