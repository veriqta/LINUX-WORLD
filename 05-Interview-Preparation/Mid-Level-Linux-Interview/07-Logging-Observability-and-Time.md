# Logging Observability and Time

> Level: Mid-Level System Administrator

## Scope

structured logs, journald persistence, rsyslog routing, rotation, metrics, cardinality, alerting and clock synchronization.

## Interview questions and model answers

### 1. What should a strong Mid-Level candidate understand about Logging Observability and Time?

A strong answer connects structured logs, journald persistence, rsyslog routing, rotation, metrics, cardinality, alerting and clock synchronization. It defines each idea, explains how the pieces interact, and finishes with a way to observe the real system rather than relying on memory.

### 2. Which commands would you use first?

Begin with read-only evidence: `journalctl --list-boots; journalctl -o json; logger -p local0.notice; logrotate -d FILE; timedatectl; chronyc tracking; ss -ulpn`. Select only the commands relevant to the symptom, narrow their scope, record time and host context, and explain what result would support or reject the current hypothesis.
...
Warning signs include Losing evidence during rotation, high-cardinality labels, alerting on symptoms without user impact. A strong candidate recognizes uncertainty, refuses unsafe shortcuts, and proposes a bounded test.
...
Build an evidence timeline across two hosts and explain how clock drift changes the conclusion. Require the candidate to narrate risk, select evidence, perform the work in an isolated lab, verify the intended condition, test one failure and restore the original state.
...
**Task:** Build an evidence timeline across two hosts and explain how clock drift changes the conclusion.
