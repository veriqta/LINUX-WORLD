...
CIDR, policy routing, neighbor discovery, TCP state, MTU, bonding, VLAN, nftables, DNS delegation and caching.

## Interview questions and model answers

### 1. What should a strong Mid-Level candidate understand about Networking Routing Firewall and DNS?

A strong answer connects CIDR, policy routing, neighbor discovery, TCP state, MTU, bonding, VLAN, nftables, DNS delegation and caching. It defines each idea, explains how the pieces interact, and finishes with a way to observe the real system rather than relying on memory.

### 2. Which commands would you use first?

Begin with read-only evidence: `ip route get ADDRESS; ip rule; ip neigh; ss -s; tracepath HOST; tcpdump -ni IFACE FILTER; nft list ruleset; dig +trace NAME`. Select only the commands relevant to the symptom, narrow their scope, record time and host context, and explain what result would support or reject the current hypothesis.
...
Warning signs include Capturing secrets, asymmetric routing, stateful firewall surprises, treating DNS as one server. A strong candidate recognizes uncertainty, refuses unsafe shortcuts, and proposes a bounded test.
...
Diagnose intermittent reachability using packet path, socket state, counters, DNS and change history. Require the candidate to narrate risk, select evidence, perform the work in an isolated lab, verify the intended condition, test one failure and restore the original state.
...
**Task:** Diagnose intermittent reachability using packet path, socket state, counters, DNS and change history.
