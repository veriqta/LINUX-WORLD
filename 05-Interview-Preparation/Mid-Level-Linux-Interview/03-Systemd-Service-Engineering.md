...
unit dependency graph, readiness, restart policy, resource control, credentials, sandboxing, timers and drop-ins.

## Interview questions and model answers

### 1. What should a strong Mid-Level candidate understand about Systemd Service Engineering?

A strong answer connects unit dependency graph, readiness, restart policy, resource control, credentials, sandboxing, timers and drop-ins. It defines each idea, explains how the pieces interact, and finishes with a way to observe the real system rather than relying on memory.

### 2. Which commands would you use first?

Begin with read-only evidence: `systemctl show UNIT; systemctl edit UNIT; systemd-analyze verify FILE; systemd-run --scope; systemd-analyze security UNIT; journalctl -o json-pretty -u UNIT`. Select only the commands relevant to the symptom, narrow their scope, record time and host context, and explain what result would support or reject the current hypothesis.
...
Warning signs include Crash loops, start-limit lockout, unsafe EnvironmentFile secrets, overriding the full vendor unit. A strong candidate recognizes uncertainty, refuses unsafe shortcuts, and proposes a bounded test.
...
Harden a custom service, preserve availability, verify effective directives and provide rollback. Require the candidate to narrate risk, select evidence, perform the work in an isolated lab, verify the intended condition, test one failure and restore the original state.
...
**Task:** Harden a custom service, preserve availability, verify effective directives and provide rollback.
