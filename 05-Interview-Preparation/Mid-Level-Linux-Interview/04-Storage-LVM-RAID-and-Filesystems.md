# Storage LVM RAID and Filesystems

> Level: Mid-Level System Administrator

## Scope

LVM layers, RAID levels, alignment, online growth, snapshots, discard, quotas, fstab and filesystem repair.

## Interview questions and model answers

### 1. What should a strong Mid-Level candidate understand about Storage LVM RAID and Filesystems?

A strong answer connects LVM layers, RAID levels, alignment, online growth, snapshots, discard, quotas, fstab and filesystem repair. It defines each idea, explains how the pieces interact, and finishes with a way to observe the real system rather than relying on memory.

### 2. Which commands would you use first?

Begin with read-only evidence: `pvs; vgs; lvs -a -o +devices; mdadm --detail DEVICE; findmnt --verify; xfs_info MOUNT; tune2fs -l DEVICE`. Select only the commands relevant to the symptom, narrow their scope, record time and host context, and explain what result would support or reject the current hypothesis.
...
Warning signs include Shrinking unsupported filesystems, snapshots without capacity monitoring, repairing mounted filesystems. A strong candidate recognizes uncertainty, refuses unsafe shortcuts, and proposes a bounded test.
...
Extend capacity safely, validate every layer, test persistence and explain the rollback boundary. Require the candidate to narrate risk, select evidence, perform the work in an isolated lab, verify the intended condition, test one failure and restore the original state.
...
**Task:** Extend capacity safely, validate every layer, test persistence and explain the rollback boundary.
