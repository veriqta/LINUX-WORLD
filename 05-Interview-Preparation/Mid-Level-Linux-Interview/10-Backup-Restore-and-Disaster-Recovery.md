...
RPO, RTO, crash consistency, application consistency, retention, encryption, immutability, restore testing and runbooks.

## Interview questions and model answers

### 1. What should a strong Mid-Level candidate understand about Backup Restore and Disaster Recovery?

A strong answer connects RPO, RTO, crash consistency, application consistency, retention, encryption, immutability, restore testing and runbooks. It defines each idea, explains how the pieces interact, and finishes with a way to observe the real system rather than relying on memory.

### 2. Which commands would you use first?

Begin with read-only evidence: `rsync -aHAX --numeric-ids; tar --xattrs --acls; sha256sum; restic check; borg check; systemctl isolate TARGET`. Select only the commands relevant to the symptom, narrow their scope, record time and host context, and explain what result would support or reject the current hypothesis.
...
Warning signs include Measuring backup success but not restore success, losing keys, restoring over the only copy. A strong candidate recognizes uncertainty, refuses unsafe shortcuts, and proposes a bounded test.
...
Design and execute a restore exercise, measure actual RPO and RTO, and record gaps. Require the candidate to narrate risk, select evidence, perform the work in an isolated lab, verify the intended condition, test one failure and restore the original state.
...
**Task:** Design and execute a restore exercise, measure actual RPO and RTO, and record gaps.
