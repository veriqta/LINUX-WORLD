# Advanced Identity PAM ACL and Sudo

> Level: Mid-Level System Administrator

## Scope

NSS, PAM, UID consistency, supplementary groups, ACL masks, sudo policy, SSH authorization and account lifecycle.

## Interview questions and model answers

### 1. What should a strong Mid-Level candidate understand about Advanced Identity PAM ACL and Sudo?

A strong answer connects NSS, PAM, UID consistency, supplementary groups, ACL masks, sudo policy, SSH authorization and account lifecycle. It defines each idea, explains how the pieces interact, and finishes with a way to observe the real system rather than relying on memory.

### 2. Which commands would you use first?

Begin with read-only evidence: `getent passwd USER; id USER; getfacl PATH; setfacl -m u:USER:rw PATH; visudo -c; pamtester SERVICE USER authenticate`. Select only the commands relevant to the symptom, narrow their scope, record time and host context, and explain what result would support or reject the current hypothesis.
...
Warning signs include Ignoring ACL mask, creating UID collisions, broad sudo wildcards, locking out remote access. A strong candidate recognizes uncertainty, refuses unsafe shortcuts, and proposes a bounded test.
...
Design time-bounded least privilege for an operations team and prove both allowed and denied actions. Require the candidate to narrate risk, select evidence, perform the work in an isolated lab, verify the intended condition, test one failure and restore the original state.
...
**Task:** Design time-bounded least privilege for an operations team and prove both allowed and denied actions.
