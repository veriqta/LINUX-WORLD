# Security Hardening SELinux and AppArmor

> Level: Mid-Level System Administrator

## Scope

MAC versus DAC, labels, domains, booleans, profiles, audit evidence, firewall default deny, patching and integrity.

## Interview questions and model answers

### 1. What should a strong Mid-Level candidate understand about Security Hardening SELinux and AppArmor?

A strong answer connects MAC versus DAC, labels, domains, booleans, profiles, audit evidence, firewall default deny, patching and integrity. It defines each idea, explains how the pieces interact, and finishes with a way to observe the real system rather than relying on memory.

### 2. Which commands would you use first?

Begin with read-only evidence: `getenforce; ls -Z PATH; ausearch -m AVC -ts recent; sealert -a FILE; aa-status; nft list ruleset; rpm -V PACKAGE; debsums PACKAGE`. Select only the commands relevant to the symptom, narrow their scope, record time and host context, and explain what result would support or reject the current hypothesis.
...
Warning signs include Disabling enforcement, using audit2allow blindly, opening broad ports, treating compliance as security. A strong candidate recognizes uncertainty, refuses unsafe shortcuts, and proposes a bounded test.
...
Resolve a denied service action by identifying intent, fixing labeling or policy, and proving least privilege. Require the candidate to narrate risk, select evidence, perform the work in an isolated lab, verify the intended condition, test one failure and restore the original state.
...
**Task:** Resolve a denied service action by identifying intent, fixing labeling or policy, and proving least privilege.
