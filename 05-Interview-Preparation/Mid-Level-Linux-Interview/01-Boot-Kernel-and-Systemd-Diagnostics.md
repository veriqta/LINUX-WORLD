# Boot Kernel and Systemd Diagnostics

> Level: Mid-Level System Administrator

## Scope

firmware, bootloader, kernel command line, initramfs, PID 1, targets, dependencies, ordering and rescue.

## Interview questions and model answers

### 1. What should a strong Mid-Level candidate understand about Boot Kernel and Systemd Diagnostics?

A strong answer connects firmware, bootloader, kernel command line, initramfs, PID 1, targets, dependencies, ordering and rescue. It defines each idea, explains how the pieces interact, and finishes with a way to observe the real system rather than relying on memory.

### 2. Which commands would you use first?

Begin with read-only evidence: `journalctl -b -1; cat /proc/cmdline; systemctl --failed; systemctl list-dependencies --reverse UNIT; systemd-analyze critical-chain; lsinitrd or lsinitramfs`. Select only the commands relevant to the symptom, narrow their scope, record time and host context, and explain what result would support or reject the current hypothesis.
...
Warning signs include Masking the root cause with repeated reboots, confusing ordering with requirement, editing generated units. A strong candidate recognizes uncertainty, refuses unsafe shortcuts, and proposes a bounded test.
...
Recover a host that reaches emergency mode after a storage change and prevent recurrence. Require the candidate to narrate risk, select evidence, perform the work in an isolated lab, verify the intended condition, test one failure and restore the original state.
...
**Task:** Recover a host that reaches emergency mode after a storage change and prevent recurrence.
