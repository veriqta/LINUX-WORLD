...
load, run queue, context switch, CPU steal, PSI, page cache, swap, OOM, disk latency, queue depth and saturation.

## Interview questions and model answers

### 1. What should a strong Mid-Level candidate understand about Performance CPU Memory and IO?

A strong answer connects load, run queue, context switch, CPU steal, PSI, page cache, swap, OOM, disk latency, queue depth and saturation. It defines each idea, explains how the pieces interact, and finishes with a way to observe the real system rather than relying on memory.

### 2. Which commands would you use first?

Begin with read-only evidence: `mpstat -P ALL 1; pidstat 1; vmstat 1; iostat -xz 1; cat /proc/pressure/{cpu,memory,io}; sar; perf stat COMMAND`. Select only the commands relevant to the symptom, narrow their scope, record time and host context, and explain what result would support or reject the current hypothesis.
...
Warning signs include Tuning before measuring, clearing cache as a fix, reading utilization without latency or queue depth. A strong candidate recognizes uncertainty, refuses unsafe shortcuts, and proposes a bounded test.
...
Separate CPU saturation, memory pressure and storage latency, then propose a testable mitigation. Require the candidate to narrate risk, select evidence, perform the work in an isolated lab, verify the intended condition, test one failure and restore the original state.
...
**Task:** Separate CPU saturation, memory pressure and storage latency, then propose a testable mitigation.
