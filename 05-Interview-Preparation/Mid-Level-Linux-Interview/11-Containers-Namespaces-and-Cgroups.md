...
image layers, container process model, namespaces, cgroups, capabilities, rootless operation, mounts and logging.

## Interview questions and model answers

### 1. What should a strong Mid-Level candidate understand about Containers Namespaces and Cgroups?

A strong answer connects image layers, container process model, namespaces, cgroups, capabilities, rootless operation, mounts and logging. It defines each idea, explains how the pieces interact, and finishes with a way to observe the real system rather than relying on memory.

### 2. Which commands would you use first?

Begin with read-only evidence: `lsns; nsenter; systemd-cgls; cat /proc/PID/cgroup; capsh --print; podman inspect CONTAINER; crictl ps`. Select only the commands relevant to the symptom, narrow their scope, record time and host context, and explain what result would support or reject the current hypothesis.
...
Warning signs include Treating containers as VMs, privileged containers, writable secrets, ignoring host resource pressure. A strong candidate recognizes uncertainty, refuses unsafe shortcuts, and proposes a bounded test.
...
Diagnose a container that is healthy internally but unreachable, checking host and namespace boundaries. Require the candidate to narrate risk, select evidence, perform the work in an isolated lab, verify the intended condition, test one failure and restore the original state.
...
**Task:** Diagnose a container that is healthy internally but unreachable, checking host and namespace boundaries.
