...
unit, target, dependency, enable versus start, daemon reload, journal, failed state and boot target.

## Interview questions and model answers

### 1. What should a strong Junior candidate understand about Systemd Services and Boot?

A strong answer connects unit, target, dependency, enable versus start, daemon reload, journal, failed state and boot target. It defines each idea, explains how the pieces interact, and finishes with a way to observe the real system rather than relying on memory.

### 2. Which commands would you use first?

Begin with read-only evidence: `systemctl status UNIT; systemctl is-enabled UNIT; systemctl cat UNIT; systemctl list-dependencies UNIT; journalctl -u UNIT -b; systemd-analyze blame`. Select only the commands relevant to the symptom, narrow their scope, record time and host context, and explain what result would support or reject the current hypothesis.
...
Warning signs include Editing vendor units, forgetting daemon-reload, restarting before validating configuration. A strong candidate recognizes uncertainty, refuses unsafe shortcuts, and proposes a bounded test.
...
Diagnose a failed service from status, unit content, dependency state and logs without blindly restarting. Require the candidate to narrate risk, select evidence, perform the work in an isolated lab, verify the intended condition, test one failure and restore the original state.
...
**Task:** Diagnose a failed service from status, unit content, dependency state and logs without blindly restarting.
