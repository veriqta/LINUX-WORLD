# Linux Foundations and Help

> Level: Junior System Administrator

## Scope

kernel, distribution, shell, terminal, GNU user space, paths, command syntax, exit status and documentation.

## Interview questions and model answers

### 1. What should a strong Junior candidate understand about Linux Foundations and Help?

A strong answer connects kernel, distribution, shell, terminal, GNU user space, paths, command syntax, exit status and documentation. It defines each idea, explains how the pieces interact, and finishes with a way to observe the real system rather than relying on memory.

### 2. Which commands would you use first?

Begin with read-only evidence: `uname -a; cat /etc/os-release; type cd; command -v ls; man hier; printf "%s\n" "$SHELL"; printf "%s\n" "$?"`. Select only the commands relevant to the symptom, narrow their scope, record time and host context, and explain what result would support or reject the current hypothesis.
...
Warning signs include Confusing the kernel with a distribution, guessing options, ignoring exit status. A strong candidate recognizes uncertainty, refuses unsafe shortcuts, and proposes a bounded test.
...
Identify the operating system and kernel, find authoritative local help, and explain how a command is resolved. Require the candidate to narrate risk, select evidence, perform the work in an isolated lab, verify the intended condition, test one failure and restore the original state.
...
**Task:** Identify the operating system and kernel, find authoritative local help, and explain how a command is resolved.
