# Packages Software and Repositories

> Level: Junior System Administrator

## Scope

package, repository, metadata, dependency, signature, installed files, update and removal.

## Interview questions and model answers

### 1. What should a strong Junior candidate understand about Packages Software and Repositories?

A strong answer connects package, repository, metadata, dependency, signature, installed files, update and removal. It defines each idea, explains how the pieces interact, and finishes with a way to observe the real system rather than relying on memory.

### 2. Which commands would you use first?

Begin with read-only evidence: `apt-cache policy PKG; dpkg -L PKG; dnf info PKG; rpm -ql PKG; command -v PROGRAM`. Select only the commands relevant to the symptom, narrow their scope, record time and host context, and explain what result would support or reject the current hypothesis.
...
Warning signs include Mixing package families, disabling signature checks, removing shared dependencies without review. A strong candidate recognizes uncertainty, refuses unsafe shortcuts, and proposes a bounded test.
...
Identify which package owns a file, inspect its source, install safely, verify, and document removal. Require the candidate to narrate risk, select evidence, perform the work in an isolated lab, verify the intended condition, test one failure and restore the original state.
...
**Task:** Identify which package owns a file, inspect its source, install safely, verify, and document removal.
