# Linux Foundations and Help: Multiple-Choice Bank

Choose the best answer, then explain why every other option is weaker.

## 1. A candidate needs to investigate kernel safely. What is the best first approach?

A. Restart the service immediately

B. Run `uname -a`, define the expected evidence and preserve the before state
...
## 2. A distribution change works now but must survive reboot. Which evidence is strongest?
...
## 3. Users are denied while an administrator succeeds in shell. What should be tested next?
...
## 4. Which result best proves recovery of terminal?
...
## 5. A measurement related to GNU user space is high. What is the best interpretation?
...
## 6. Which change practice is safest for paths?
...
## 7. A candidate needs to investigate command syntax safely. What is the best first approach?

A. Restart the service immediately

B. Run `printf "%s\n" "$?"`, define the expected evidence and preserve the before state
...
## 8. A exit status and documentation change works now but must survive reboot. Which evidence is strongest?
