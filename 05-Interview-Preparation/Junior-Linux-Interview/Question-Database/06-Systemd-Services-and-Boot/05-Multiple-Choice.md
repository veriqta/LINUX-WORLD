# Systemd Services and Boot: Multiple-Choice Bank

Choose the best answer, then explain why every other option is weaker.

## 1. A candidate needs to investigate unit safely. What is the best first approach?

A. Restart the service immediately

B. Run `systemctl status UNIT`, define the expected evidence and preserve the before state
...
## 2. A target change works now but must survive reboot. Which evidence is strongest?
...
## 3. Users are denied while an administrator succeeds in dependency. What should be tested next?
...
## 4. Which result best proves recovery of enable versus start?
...
## 5. A measurement related to daemon reload is high. What is the best interpretation?
...
## 6. Which change practice is safest for journal?
...
## 7. A candidate needs to investigate failed state and boot target safely. What is the best first approach?
