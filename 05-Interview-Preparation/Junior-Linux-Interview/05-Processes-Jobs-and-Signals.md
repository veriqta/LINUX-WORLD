...
PID, PPID, process state, foreground, background, job control, signals, nice value and open files.

## Interview questions and model answers

### 1. What should a strong Junior candidate understand about Processes Jobs and Signals?

A strong answer connects PID, PPID, process state, foreground, background, job control, signals, nice value and open files. It defines each idea, explains how the pieces interact, and finishes with a way to observe the real system rather than relying on memory.

### 2. Which commands would you use first?

Begin with read-only evidence: `ps -ef; ps -o pid,ppid,stat,ni,cmd -p PID; jobs -l; kill -TERM PID; pgrep -a NAME; lsof -p PID`. Select only the commands relevant to the symptom, narrow their scope, record time and host context, and explain what result would support or reject the current hypothesis.
...
Warning signs include Starting with SIGKILL, matching the wrong process, confusing high load with CPU use. A strong candidate recognizes uncertainty, refuses unsafe shortcuts, and proposes a bounded test.
...
Find a process, inspect its parent and files, request graceful termination, and prove the correct process stopped. Require the candidate to narrate risk, select evidence, perform the work in an isolated lab, verify the intended condition, test one failure and restore the original state.
...
**Task:** Find a process, inspect its parent and files, request graceful termination, and prove the correct process stopped.
