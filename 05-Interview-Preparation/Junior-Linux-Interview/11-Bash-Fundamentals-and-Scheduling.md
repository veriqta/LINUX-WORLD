# Bash Fundamentals and Scheduling

> Level: Junior System Administrator

## Scope

shebang, variable, quoting, positional argument, condition, loop, function, exit code, cron and systemd timer.

## Interview questions and model answers

### 1. What should a strong Junior candidate understand about Bash Fundamentals and Scheduling?

A strong answer connects shebang, variable, quoting, positional argument, condition, loop, function, exit code, cron and systemd timer. It defines each idea, explains how the pieces interact, and finishes with a way to observe the real system rather than relying on memory.

### 2. Which commands would you use first?

Begin with read-only evidence: `bash -n SCRIPT; shellcheck SCRIPT; printf "%q\n" "$value"; crontab -l; systemctl list-timers; logger MESSAGE`. Select only the commands relevant to the symptom, narrow their scope, record time and host context, and explain what result would support or reject the current hypothesis.
...
Warning signs include Unquoted expansion, parsing human output, missing strict error handling, assuming cron environment. A strong candidate recognizes uncertainty, refuses unsafe shortcuts, and proposes a bounded test.
...
Write a small idempotent health script with validation, useful errors, logging and a meaningful exit code. Require the candidate to narrate risk, select evidence, perform the work in an isolated lab, verify the intended condition, test one failure and restore the original state.
...
**Task:** Write a small idempotent health script with validation, useful errors, logging and a meaningful exit code.
