...
least privilege, patching, firewall, secure SSH, backup, restore, evidence, hypothesis and escalation.

## Interview questions and model answers

### 1. What should a strong Junior candidate understand about Security Backup and Troubleshooting?

A strong answer connects least privilege, patching, firewall, secure SSH, backup, restore, evidence, hypothesis and escalation. It defines each idea, explains how the pieces interact, and finishes with a way to observe the real system rather than relying on memory.

### 2. Which commands would you use first?

Begin with read-only evidence: `sudo -l; getfacl PATH; sshd -T; firewall-cmd --list-all; ufw status verbose; tar -tf ARCHIVE; sha256sum FILE`. Select only the commands relevant to the symptom, narrow their scope, record time and host context, and explain what result would support or reject the current hypothesis.
...
Warning signs include Calling an untested archive a backup, changing many things at once, publishing sensitive logs. A strong candidate recognizes uncertainty, refuses unsafe shortcuts, and proposes a bounded test.
...
Prove a backup can restore, confirm access controls, investigate one failure, recover and record evidence. Require the candidate to narrate risk, select evidence, perform the work in an isolated lab, verify the intended condition, test one failure and restore the original state.
...
**Task:** Prove a backup can restore, confirm access controls, investigate one failure, recover and record evidence.
