...
standard input, output, error, pipes, redirection, grep, cut, sort, uniq, tr, head, tail and tee.

## Interview questions and model answers

### 1. What should a strong Junior candidate understand about Text Processing and Redirection?

A strong answer connects standard input, output, error, pipes, redirection, grep, cut, sort, uniq, tr, head, tail and tee. It defines each idea, explains how the pieces interact, and finishes with a way to observe the real system rather than relying on memory.

### 2. Which commands would you use first?

Begin with read-only evidence: `grep -n PATTERN FILE; cut -d: -f1 /etc/passwd; sort FILE | uniq -c; command >out 2>err; tail -F LOG`. Select only the commands relevant to the symptom, narrow their scope, record time and host context, and explain what result would support or reject the current hypothesis.
...
Warning signs include Using grep when structured parsing is required, overwriting with >, hiding errors with broad redirection. A strong candidate recognizes uncertainty, refuses unsafe shortcuts, and proposes a bounded test.
...
Extract a field, count repeated values, preserve errors separately, and explain every stage of a pipeline. Require the candidate to narrate risk, select evidence, perform the work in an isolated lab, verify the intended condition, test one failure and restore the original state.
...
**Task:** Extract a field, count repeated values, preserve errors separately, and explain every stage of a pipeline.
