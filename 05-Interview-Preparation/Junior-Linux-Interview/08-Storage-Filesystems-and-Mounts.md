...
block device, partition, filesystem, mount point, UUID, inode, free space and swap.

## Interview questions and model answers

### 1. What should a strong Junior candidate understand about Storage Filesystems and Mounts?

A strong answer connects block device, partition, filesystem, mount point, UUID, inode, free space and swap. It defines each idea, explains how the pieces interact, and finishes with a way to observe the real system rather than relying on memory.

### 2. Which commands would you use first?

Begin with read-only evidence: `lsblk -f; findmnt; df -hT; df -i; blkid; swapon --show; du -xhd1 PATH`. Select only the commands relevant to the symptom, narrow their scope, record time and host context, and explain what result would support or reject the current hypothesis.
...
Warning signs include Formatting the wrong device, confusing df with du, ignoring inode exhaustion, editing fstab without testing. A strong candidate recognizes uncertainty, refuses unsafe shortcuts, and proposes a bounded test.
...
Diagnose a full filesystem and distinguish used blocks, deleted-open files, inodes and mount problems. Require the candidate to narrate risk, select evidence, perform the work in an isolated lab, verify the intended condition, test one failure and restore the original state.
...
**Task:** Diagnose a full filesystem and distinguish used blocks, deleted-open files, inodes and mount problems.
