# Logs Monitoring and Resources

> Level: Junior System Administrator

## Scope

journal, syslog, log rotation, CPU, memory, swap, load, disk latency and time window.

## Interview questions and model answers

### 1. What should a strong Junior candidate understand about Logs Monitoring and Resources?

A strong answer connects journal, syslog, log rotation, CPU, memory, swap, load, disk latency and time window. It defines each idea, explains how the pieces interact, and finishes with a way to observe the real system rather than relying on memory.

### 2. Which commands would you use first?

Begin with read-only evidence: `journalctl --since "15 min ago"; free -h; uptime; vmstat 1 5; top; dmesg --level=err,warn; timedatectl`. Select only the commands relevant to the symptom, narrow their scope, record time and host context, and explain what result would support or reject the current hypothesis.
...
Warning signs include Reading logs without timestamps, calling cache used memory, treating load as CPU percentage. A strong candidate recognizes uncertainty, refuses unsafe shortcuts, and proposes a bounded test.
...
Collect a five-minute evidence bundle that another administrator can use without exposing secrets. Require the candidate to narrate risk, select evidence, perform the work in an isolated lab, verify the intended condition, test one failure and restore the original state.
...
**Task:** Collect a five-minute evidence bundle that another administrator can use without exposing secrets.
