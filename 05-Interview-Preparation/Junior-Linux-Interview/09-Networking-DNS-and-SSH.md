...
interface, address, route, gateway, port, socket, DNS resolver, TCP, UDP and SSH host verification.

## Interview questions and model answers

### 1. What should a strong Junior candidate understand about Networking DNS and SSH?

A strong answer connects interface, address, route, gateway, port, socket, DNS resolver, TCP, UDP and SSH host verification. It defines each idea, explains how the pieces interact, and finishes with a way to observe the real system rather than relying on memory.

### 2. Which commands would you use first?

Begin with read-only evidence: `ip -brief address; ip route; ss -lntup; getent hosts NAME; resolvectl status; ping -c 3 HOST; ssh -vv USER@HOST`. Select only the commands relevant to the symptom, narrow their scope, record time and host context, and explain what result would support or reject the current hypothesis.
...
Warning signs include Testing only ping, exposing SSH, ignoring host keys, changing firewall rules without preserving access. A strong candidate recognizes uncertainty, refuses unsafe shortcuts, and proposes a bounded test.
...
Trace a connection from local address through route, name resolution, socket state and remote service. Require the candidate to narrate risk, select evidence, perform the work in an isolated lab, verify the intended condition, test one failure and restore the original state.
...
**Task:** Trace a connection from local address through route, name resolution, socket state and remote service.
