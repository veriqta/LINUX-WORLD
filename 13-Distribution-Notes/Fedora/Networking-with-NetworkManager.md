...
This guide explains networking with networkmanager as it applies specifically to Fedora. It records distribution differences without repeating the general Linux course.
...
NetworkManager is standard on common editions. systemd-resolved use depends on edition and configuration; verify resolver ownership. Inspect the active manager, generated runtime state, routes, addresses, and resolver before editing configuration.

```bash
ip -brief address
ip route
nmcli general status; nmcli connection show
resolvectl status 2>/dev/null || { readlink -f /etc/resolv.conf; cat /etc/resolv.conf; }
