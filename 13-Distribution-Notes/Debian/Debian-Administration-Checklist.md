# Debian Administration Checklist
...
This guide explains debian administration checklist as it applies specifically to Debian. It records distribution differences without repeating the general Linux course.
...
This topic must be interpreted through the Debian release, edition, installed packages, enabled repositories, security policy, and local configuration. Stable Linux concepts transfer across distributions, but package names, defaults, paths, service units, and supported workflows may not.

```bash
cat /etc/os-release
uname -r
systemctl --version | head -1
dpkg-query -W systemd 2>/dev/null || true
```
