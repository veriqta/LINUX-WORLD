...
This guide explains cloud images and cloud init as it applies specifically to Debian. It records distribution differences without repeating the general Linux course.
...
## Cloud image operations

Cloud images may differ from installation-media systems in users, partition growth, datasource behavior, package selection, networking, SSH policy, and vendor agents.

```bash
cloud-init status --long 2>/dev/null || true
cloud-id 2>/dev/null || true
systemctl status cloud-init-local.service cloud-init.service --no-pager 2>/dev/null || true
findmnt /
```

Never rerun cloud-init cleanup commands on a production instance without understanding identity regeneration. Test image changes by launching a new instance, not by assuming an existing host represents the image.
