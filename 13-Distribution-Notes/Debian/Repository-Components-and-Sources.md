# Repository Components and Sources
...
This guide explains repository components and sources as it applies specifically to Debian. It records distribution differences without repeating the general Linux course.

## Learning objectives

- Identify the Debian-specific tools, paths, defaults, and support boundaries.
...
Confirm that the system is Debian, identify its exact release and architecture, take an appropriate backup or snapshot, preserve remote recovery access, and consult the release-specific official documentation.

## Repository model

Repository configuration controls trust and software provenance. Debian administrators should inventory enabled sources before installing or upgrading anything.

```bash
apt-cache policy; grep -Rhs -- "^deb" /etc/apt/sources.list /etc/apt/sources.list.d 2>/dev/null
```

Do not add a third-party source merely because a package is missing. Verify ownership, signing keys, release compatibility, update practices, namespace collisions, removal procedure, and incident-response contact. Test repository priority or pinning in a disposable system. Never disable signature verification to bypass a repository problem.
...
- Disabling AppArmor may be available or enabled depending on installation and release or the firewall to hide a configuration error.
...
Create a disposable Debian virtual machine. Capture release metadata, repository state, installed package ownership, active network manager, resolver ownership, firewall state, mandatory-access-control state, filesystem layout, failed units, and recent warnings. Explain which observations are distribution defaults and which are local configuration.
...
- [Debian release and lifecycle information](https://www.debian.org/releases/)
