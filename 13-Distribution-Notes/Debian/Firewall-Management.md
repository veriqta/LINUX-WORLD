...
This guide explains firewall management as it applies specifically to Debian. It records distribution differences without repeating the general Linux course.
...
nftables is the modern packet-filtering interface; installed frontends vary. Inventory the current rules, default policy, zones or chains, IPv4 and IPv6 behavior, persistence, and management-access path before a change.
...
Use console access or an automatic rollback when changing remote firewall rules. Test allowed and denied paths from another host. A listening socket does not prove network reachability, and an open firewall rule does not prove an application is listening.
