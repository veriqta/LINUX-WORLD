...
This guide explains services and systemd as it applies specifically to Debian. It records distribution differences without repeating the general Linux course.
...
The distribution uses systemd on the releases covered here. Package installation, unit enablement, process state, readiness, network exposure, and user-facing health are separate conditions.

```bash
systemctl list-unit-files --type=service
systemctl status SERVICE.service --no-pager -l
systemctl cat SERVICE.service
systemctl show SERVICE.service -p ActiveState -p SubState -p FragmentPath
journalctl -u SERVICE.service --since '-30 min' --no-pager
```

Use `systemctl reload` only when the service documents reload support. Validate configuration before restart and verify a representative request afterward.
