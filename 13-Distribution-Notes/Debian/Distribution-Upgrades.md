...
This guide explains distribution upgrades as it applies specifically to Debian. It records distribution differences without repeating the general Linux course.
...
## Release and update control

The supported administrative path is APT using the official release notes and a supported release path. Read the source and target release notes, confirm lifecycle status from the official source, update the current release fully, test backups, inventory third-party repositories, validate free space, and rehearse rollback or rebuild.
...
Do not infer a supported upgrade path from version numbers alone. Kernel, bootloader, database, repository, application-stream, and configuration changes can alter the correct procedure.
