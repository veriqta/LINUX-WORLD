...
This guide explains selinux as it applies specifically to Rocky-Linux. It records distribution differences without repeating the general Linux course.
...
The relevant control for this distribution is SELinux. Treat a denial as evidence of a policy mismatch, not a reason to disable enforcement.

```bash
getenforce; sestatus
journalctl -k --since '-30 min' --no-pager | tail -200
```

Verify labels, profiles, domains, paths, ports, booleans, and recent configuration changes. Make the narrowest justified policy change. Record the denied operation and test that unrelated access remains blocked.
