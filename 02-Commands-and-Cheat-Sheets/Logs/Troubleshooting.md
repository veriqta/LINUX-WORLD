...
| --- | --- |
| No logs | Check whether the process reached startup, output destination, logging level, permissions, rate limits, and journal persistence. |
| Logs disappeared after reboot | Check volatile versus persistent journal storage and rotation policy. |
| Timestamps disagree | Check time synchronization, timezone, container settings, and application formatting. |
| Log file grows uncontrollably | Fix verbosity or error loop, validate rotation, compression, retention, disk alerts, and reopen behavior. |
| Search misses known event | Check rotated and compressed files, case, encoding, multiline events, fields, and time window. |
