# Operations and Investigation Leads

This guide explains how operations and investigation leads supports the ability to reduce harm through structured command, investigation, mitigation, and communication.
...
- Explain operations and investigation leads in an operational context.
...
Operations and Investigation Leads is not complete when a command succeeds or a document exists. It is complete when the intended service outcome is demonstrated, the risk is bounded, the evidence is preserved, and the next responsible person can understand the resulting state.
...
```bash
# Collect volatile facts before invasive action
date -u +%Y-%m-%dT%H:%M:%SZ
uptime
who
ps -eo pid,ppid,user,state,lstart,cmd --sort=lstart
ss -plant
...
Close only after the service is stable for an appropriate observation period and monitoring is trustworthy. Escalate when parallel work becomes unsafe or the team loses a shared view of impact and priorities. Handover must state the current impact, actions completed, evidence, remaining hypotheses, risks, and next decision deadline.
