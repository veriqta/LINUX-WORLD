...
This playbook coordinates response to a network partition across technical and organizational boundaries.
...
- Explain network partition in an operational context.
...
- **Activate when:** Required components cannot communicate consistently across a failure boundary.
- **Containment priorities:** Prevent split-brain writes. Identify the isolated side. Prefer quorum-safe operation.
- **Recovery proof:** Verify Bidirectional reachability, routes, name resolution, quorum, replication lag, and transactions.
