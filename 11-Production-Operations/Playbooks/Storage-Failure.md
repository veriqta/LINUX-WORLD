...
This playbook coordinates response to a storage failure across technical and organizational boundaries.
...
- Explain storage failure in an operational context.
...
- **Activate when:** Latency, errors, capacity, corruption indicators, or device loss threaten service or data.
- **Containment priorities:** Reduce writes. Isolate failed media. Protect replicas and backups. Do not repair mounted or unconfirmed filesystems.
- **Recovery proof:** Verify Kernel errors, filesystem state, data checks, replica health, latency, durability, and restart behavior.
