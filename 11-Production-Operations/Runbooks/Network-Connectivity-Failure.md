...
This runbook provides a controlled response when an application cannot reach an approved endpoint.
...
- Explain network-connectivity-failure in an operational context.
...
```bash
ip addr; ip route; getent hosts HOST; ss -s
```
...
```bash
ip route replace ROUTE via GATEWAY
```
...
```bash
ip route get DESTINATION; timeout 5 bash -c "</dev/tcp/HOST/PORT"
```
