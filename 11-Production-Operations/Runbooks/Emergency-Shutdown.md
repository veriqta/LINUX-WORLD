...
This runbook provides a controlled response when continuing operation presents immediate safety, integrity, or containment risk.
...
- Explain emergency-shutdown in an operational context.
...
```bash
date -u; who; systemctl --failed; ss -plant
```
...
```bash
Verify out-of-band power state and affected-service failover from another authorized system
```
