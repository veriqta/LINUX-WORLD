...
This runbook provides a controlled response when an approved reboot is required for maintenance or recovery.
...
- Explain server-reboot-procedure in an operational context.
...
```bash
uptime; who; systemctl --failed; systemd-inhibit --list
```
...
```bash
uptime; systemctl --failed; systemctl is-active CRITICAL_SERVICE.service
```
