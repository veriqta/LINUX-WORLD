...
This runbook provides a controlled response when an approved capacity change requires filesystem growth.
...
- Explain filesystem-expansion in an operational context.
...
```bash
lsblk -f; findmnt /MOUNT; df -hT /MOUNT
```
...
```bash
lvextend -L +SIZE /dev/VG/LV && resize2fs /dev/VG/LV
```
