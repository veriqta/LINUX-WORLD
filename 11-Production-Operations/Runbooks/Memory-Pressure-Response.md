...
This runbook provides a controlled response when memory pressure, reclaim, or OOM events threaten availability.
...
- Explain memory-pressure-response in an operational context.
...
```bash
cat /proc/pressure/memory; free -h
```
...
```bash
systemctl stop APPROVED_NONCRITICAL_SERVICE.service
```
...
```bash
cat /proc/pressure/memory; journalctl -k -g "oom|Out of memory"
```
