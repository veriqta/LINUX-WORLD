...
This runbook provides a controlled response when a certificate is expired or near its rotation threshold.
...
- Explain certificate-expiration in an operational context.
...
```bash
install -m 0644 NEW_CERT.pem APPROVED_PATH/CERT.pem && systemctl reload SERVICE.service
```
...
```bash
openssl s_client -connect HOST:443 -servername NAME </dev/null 2>/dev/null | openssl x509 -noout -dates -issuer
```
