...
This runbook provides a controlled response when a configuration change violates a guardrail.
...
```bash
git diff; systemctl status SERVICE.service --no-pager
```
...
```bash
install -m MODE KNOWN_GOOD.conf TARGET.conf && systemctl reload SERVICE.service
```
...
```bash
VALIDATOR_COMMAND && systemctl is-active SERVICE.service
```
