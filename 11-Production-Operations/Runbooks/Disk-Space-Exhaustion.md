...
This runbook provides a controlled response when filesystem utilization crosses an approved response threshold.
...
- Explain disk-space-exhaustion in an operational context.
...
```bash
df -hT; du -x -d1 /PATH 2>/dev/null | sort -n
```
...
```bash
journalctl --vacuum-time=APPROVED_RETENTION
```
