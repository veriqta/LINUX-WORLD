...
This runbook provides a controlled response when inode usage prevents new files despite available bytes.
...
- Explain inode-exhaustion in an operational context.
...
```bash
df -i; find /PATH -xdev -type f -printf "%h\n" | sort | uniq -c | sort -n | tail
```
...
```bash
find /APPROVED/CACHE -xdev -type f -mtime +RETENTION -print
```
