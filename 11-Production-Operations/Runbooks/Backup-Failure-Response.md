...
This runbook provides a controlled response when a scheduled backup fails or misses its recovery window.
...
- Explain backup-failure-response in an operational context.
...
```bash
systemctl status BACKUP.service --no-pager; journalctl -u BACKUP.service --since -1d
```
...
```bash
systemctl is-active BACKUP.service; sha256sum BACKUP_ARTIFACT
```
