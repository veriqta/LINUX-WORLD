# Hardware Clocks and System Time

This article explains hardware clocks and system time as part of Linux internals. It focuses on why the mechanism exists, which kernel and userspace components participate, how state changes, and how an administrator can observe it safely.
...
- Explain how hardware clocks and system time helps Linux maintain useful clocks, correct wall time and deliver timed events.
- Identify the principal internal objects: hardware clock, clocksource, wall clock, monotonic clocks, NTP discipline, timers, timer queues and timer file descriptors.
...
This mechanism exists to maintain useful clocks, correct wall time and deliver timed events. Without it, Linux could not provide a consistent and protected operating environment across different workloads and hardware. The implementation balances correctness, isolation, latency, throughput, fairness, compatibility and observability.

## Internal objects and participants

The important objects include hardware clock, clocksource, wall clock, monotonic clocks, NTP discipline, timers, timer queues and timer file descriptors. Names exposed by tools are views of kernel state, not necessarily the complete internal structure.
...
## Control or data flow
    ...
    hardware counter -> clocksource -> kernel timekeeping -> clock APIs -> timer expiry -> wakeup or signal
...
- Use monotonic time for elapsed duration and wall time for human correlation.
- Record UTC, clock source, synchronization state and known offset in investigations.
- Time correction can be stepped or slewed; application effects depend on the clock API used.

## Kernel and userspace interfaces

Useful interfaces include /sys/devices/system/clocksource, timedate state, adjtimex data and process timer descriptors. These interfaces can be version-dependent, namespace-relative, privileged or transient. Capture the kernel release, distribution, architecture and tool version with evidence.
...
Run commands only on an authorized system and protect output that reveals topology, workloads or identities:
    ...
    adjtimex --print 2>/dev/null | head
    ls -l /proc/$$/fd
...
- Hardware Clocks and System Time is not fully explained by one command or one counter.
...
1. What problem does hardware clocks and system time solve?
