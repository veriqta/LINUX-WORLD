# TCP Congestion and Retransmission

This article explains tcp congestion and retransmission as part of Linux internals. It focuses on why the mechanism exists, which kernel and userspace components participate, how state changes, and how an administrator can observe it safely.
...
- Explain how tcp congestion and retransmission helps Linux move application data through sockets, protocols, policy, routing, queues and network devices.
- Identify the principal internal objects: sockets, sk_buffs, protocol control blocks, routing tables, neighbor entries, netfilter state, qdiscs and devices.
...
This mechanism exists to move application data through sockets, protocols, policy, routing, queues and network devices. Without it, Linux could not provide a consistent and protected operating environment across different workloads and hardware. The implementation balances correctness, isolation, latency, throughput, fairness, compatibility and observability.

## Internal objects and participants

The important objects include sockets, sk_buffs, protocol control blocks, routing tables, neighbor entries, netfilter state, qdiscs and devices. Names exposed by tools are views of kernel state, not necessarily the complete internal structure.
...
## Control or data flow
    ...
    send call -> socket -> transport -> IP and route -> netfilter -> qdisc -> driver -> NIC; receive reverses the path
...
- Identify the network namespace and interface before interpreting routes or sockets.
- Follow both directions and include policy, routing, neighbor resolution and application state.
- Packet capture proves observed traffic at a location, not end-to-end application correctness.

## Kernel and userspace interfaces

Useful interfaces include /proc/net, rtnetlink, network namespaces, conntrack, qdiscs and packet capture. These interfaces can be version-dependent, namespace-relative, privileged or transient. Capture the kernel release, distribution, architecture and tool version with evidence.
...
- TCP Congestion and Retransmission is not fully explained by one command or one counter.
...
1. What problem does tcp congestion and retransmission solve?
