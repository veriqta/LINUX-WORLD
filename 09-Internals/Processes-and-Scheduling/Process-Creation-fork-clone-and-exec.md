# Process Creation fork clone and exec

This article explains process creation fork clone and exec as part of Linux internals. It focuses on why the mechanism exists, which kernel and userspace components participate, how state changes, and how an administrator can observe it safely.
...
- Explain how process creation fork clone and exec helps Linux represent execution and decide when and where runnable tasks use CPUs.
...
- A library function may issue several system calls or avoid a system call through the vDSO.
- Interpret calls using the process ABI, namespace, file descriptors and return values.
- Tracing changes timing and can expose sensitive arguments.
...
- Process Creation fork clone and exec is not fully explained by one command or one counter.
...
1. What problem does process creation fork clone and exec solve?
