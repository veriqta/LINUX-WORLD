# Application to Storage Path

This article explains application to storage path as part of Linux internals. It focuses on why the mechanism exists, which kernel and userspace components participate, how state changes, and how an administrator can observe it safely.
...
- Explain how application to storage path helps Linux move data between applications, caches, block layers, mappings, drivers and durable media.
...
- Application to Storage Path is not fully explained by one command or one counter.
...
1. What problem does application to storage path solve?
